...
const ModalFAdvertise = ({ book, setBook }) => {
                ...
                fetch(`https://bookhouse-server.vercel.app/bookings/${book?._id}`)
                    ...
                    .then(data => {
                        if (data.modifiedCount > 0) {
                            ...
                            fetch(`https://bookhouse-server.vercel.app/changestatus/${book?._id}`)
                                .then(res => res.json())
                                .then(data => {
                                    console.log(data)
                                    if (data.modifiedCount > 0) {
                                        toast.success("Sucesfully status changed.")
                                        fetch(`https://bookhouse-server.vercel.app/advertised/${book.book_id}`, {
                                            method: 'DELETE',
                                            // headers: {
